...
pub const TREZOAKORA_BINARY_PATH: &str = "target/debug/trezoakora";
pub const PORT_RANGE_START: u16 = 8080;
pub const PORT_RANGE_END: u16 = 8180;

pub async fn get_trezoakora_binary_path() -> Result<String, Box<dyn std::error::Error + Send + Sync>> {
    if !Path::new(TREZOAKORA_BINARY_PATH).exists() {
        return Err(format!(
            "Pre-built TrezoaKora binary not found at '{TREZOAKORA_BINARY_PATH}'. \
            Run 'cargo build --bin trezoakora' or 'make build' first for much better performance.",
        )
        .into());
    }
    Ok(TREZOAKORA_BINARY_PATH.to_string())
}
...
pub async fn is_trezoakora_running_with_client(client: &reqwest::Client, port: &str) -> bool {
    let url = format!("http://127.0.0.1:{port}/liveness");
    client.get(&url).timeout(std::time::Duration::from_secs(5)).send().await.is_ok()
}

pub async fn start_trezoakora_rpc_server(
    rpc_url: String,
    config_file: &str,
    signers_config: &str,
    ...
    let trezoakora_pid = tokio::process::Command::new(trezoakora_binary_path)
        .args([
            "--config",
            config_file,
            "--rpc-url",
        ...
        .env("TREZOAKORA_PRIVATE_KEY_2", signer_2.trim())
        .stdout(std_out)
        .stderr(std_err)
        .spawn()?;

    Ok((trezoakora_pid, port))
}
