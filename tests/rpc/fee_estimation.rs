...
async fn test_estimate_transaction_fee_with_fee_token_legacy() {
    ...
    // Mocked price DEFAULT_MOCKED_PRICE is 0.001, so 0.001 TRZ per usdc
    // Fee in lamport is 10050
    // 10000 lamports -> 0.00001 TRZ -> 0.00001 / 0.001 (trz per usdc) = 0.01 usdc
...
/// TPL token operations, compute budget, and priority fees
#[tokio::test]
async fn test_estimate_fee_comprehensive_with_token_accounts_creation() {
    ...
    let manual_tpl_account1 = Keypair::new();
...
#[tokio::test]
async fn test_estimate_fee_with_tpl_token_transfer_from_fee_payer() {
    let ctx = TestContext::new().await.expect("Failed to create test context");
    ...
    let mint_instruction = tpl_token_interface::instruction::mint_to(
        &tpl_token_interface::id(),
        &usdc_mint,
        &fee_payer_ata,
    ...
    // - TPL token outflow #1: 1 USDC × 0.001 TRZ/USDC = 1,000,000 lamports
    // - TPL token outflow #2: 3 USDC × 0.001 TRZ/USDC = 3,000,000 lamports
    ...
    assert_eq!(
        ...
        "Fee should include TPL token outflow value. Got {fee_lamports} expected 4_005_050",
    );
}
