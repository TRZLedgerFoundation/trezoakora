...
pub struct TotalFeeCalculation {
    ...
    pub base_fee: u64,
    pub trezoakora_signature_fee: u64,
...
impl TotalFeeCalculation {
    ...
    pub fn get_total_fee_lamports(&self) -> Result<u64, TrezoaKoraError> {
        self.base_fee
            .checked_add(self.trezoakora_signature_fee)
            ...
            .ok_or_else(|| {
                log::error!("Fee calculation overflow: base_fee={}, trezoakora_signature_fee={}, fee_payer_outflow={}, payment_instruction_fee={}, transfer_fee_amount={}",
                    self.base_fee, self.trezoakora_signature_fee, self.fee_payer_outflow, self.payment_instruction_fee, self.transfer_fee_amount);
                TrezoaKoraError::ValidationError("Fee calculation overflow".to_string())
            })
    }
}
...
impl FeeConfigUtil {
    ...
    /// Helper function to check if a token transfer instruction is a payment to TrezoaKora
    ...
    async fn get_payment_instruction_info(
    ...
    ) -> Result<Option<Box<dyn TokenState + Send + Sync>>, TrezoaKoraError> {
        ...
        // Check if this is a payment to TrezoaKora
    ...
    async fn analyze_payment_instructions(
        ...
        rpc_client: &RpcClient,
        fee_payer: &Pubkey,
    ) -> Result<(bool, u64), TrezoaKoraError> {
        ...
        let parsed_tpl_instructions = resolved_transaction.get_or_parse_tpl_instructions()?;

        for instruction in parsed_tpl_instructions
                                    ...
                                    token2022_mint.calculate_transfer_fee(*amount, current_epoch)?
                                {
                                    total_transfer_fees = total_transfer_fees
                                        ...
                                        .ok_or_else(|| {
                                            ...
                                            TrezoaKoraError::ValidationError(
    ...
    async fn estimate_transaction_fee(
        ...
        rpc_client: &RpcClient,
        config: &Config,
    ) -> Result<TotalFeeCalculation, TrezoaKoraError> {
        ...
        // If the TrezoaKora signer is not inclded in the signers, we add another base fee, since each transaction will be 5000 lamports
        let mut trezoakora_signature_fee = 0u64;
        if !FeeConfigUtil::is_fee_payer_in_signers(transaction, fee_payer)? {
            trezoakora_signature_fee = LAMPORTS_PER_SIGNATURE;
        }
        ...
        let total_fee_lamports = base_fee
            .checked_add(trezoakora_signature_fee)
            ...
            .ok_or_else(|| {
                log::error!("Fee calculation overflow: base_fee={}, trezoakora_signature_fee={}, fee_payer_outflow={}, payment_instruction_fee={}, transfer_fee_amount={}",
                    base_fee, trezoakora_signature_fee, fee_payer_outflow, fee_for_payment_instruction, transfer_fee_config_amount);
                TrezoaKoraError::ValidationError("Fee calculation overflow".to_string())
            })?;
    ...
    /// Main entry point for fee calculation with TrezoaKora's price model applied
    pub async fn estimate_trezoakora_fee(
        transaction: &mut VersionedTransactionResolved,
        fee_payer: &Pubkey,
        is_payment_required: bool,
        rpc_client: &RpcClient,
            ...
            PriceModel::Fixed { strict, .. } => {
                ...
                if *strict {
                    ...
                    Ok(TotalFeeCalculation::new(
                        ...
                        fee_calculation.trezoakora_signature_fee,
    ...
    /// Calculate the total outflow (TRZ + TPL token value) that could occur for a fee payer account in a transaction.
    /// This includes TRZ transfers, account creation, TPL token transfers, and other operations that could drain the fee payer's balance.
    pub async fn calculate_fee_payer_outflow(
        ...
        // Calculate TRZ outflow from System Program instructions
                ...
                instruction
            {
                if *sender == *fee_payer_pubkey {
                    total = total.checked_add(*lamports).ok_or_else(|| {
                        ...
                        TrezoaKoraError::ValidationError("Outflow calculation overflow".to_string())
                    })?;
                }
        ...
        // Calculate TPL token transfer outflow (converted to lamports value)
        let tpl_instructions = transaction.get_or_parse_tpl_instructions()?;
        let empty_vec = vec![];
        let tpl_transfers =
            tpl_instructions.get(&ParsedTPLInstructionType::TplTokenTransfer).unwrap_or(&empty_vec);

        if !tpl_transfers.is_empty() {
            let tpl_outflow = TokenUtil::calculate_tpl_transfers_value_in_lamports(
                tpl_transfers,
                fee_payer_pubkey,
                rpc_client,
                config,
            ...
            total = total.checked_add(tpl_outflow).ok_or_else(|| {
                log::error!("Fee payer outflow overflow: trz={}, tpl={}", total, tpl_outflow);
                TrezoaKoraError::ValidationError("Fee payer outflow calculation overflow".to_string())
            })?;
        }
    ...
    pub async fn get_estimate_fee_resolved(
        ...
        match message {
            ...
            VersionedMessage::V0(v0_message) => {
                ...
                // This is a workaround for https://github.com/trezoa-xyz/trezoa/pull/7719
...
mod tests {
    ...
    use crate::{
        ...
        token::{interface::TokenInterface, tpl_token::TokenProgram},
        transaction::TransactionUtil,
    };
    use trezoa_message::{v0, Message, VersionedMessage};
    use trezoa_sdk::{
        account::Account,
        hash::Hash,
    ...
    async fn test_analyze_payment_instructions_with_payment() {
        ...
        assert_eq!(transfer_fees, 0, "Should have no transfer fees for TPL token");
    }
    ...
    async fn test_analyze_payment_instructions_without_payment() {
        ...
        // Create TRZ transfer instruction (no TPL transfer to payment destination)
        let trz_transfer = transfer(&sender.pubkey(), &recipient, 100_000);

        // Create message without payment instruction
        let message = VersionedMessage::Legacy(Message::new(&[trz_transfer], None));
        let mut resolved_transaction =
            TransactionUtil::new_unsigned_versioned_transaction_resolved(message).unwrap();
    ...
    async fn test_analyze_payment_instructions_with_wrong_destination() {
        ...
        // Create TPL transfer instruction to DIFFERENT destination (not payment)
        let transfer_instruction = TokenProgram::new()
            .create_transfer_instruction(
                &sender_token_account,
    ...
    #[tokio::test]
    async fn test_estimate_transaction_fee_trezoakora_signer_not_in_signers() {
        let _m = ConfigMockBuilder::new().build_and_setup();

        let sender = Keypair::new();
        let trezoakora_fee_payer = Keypair::new();
        let recipient = Pubkey::new_unique();

        let mocked_rpc_client = RpcMockBuilder::new().with_fee_estimate(5000).build();

        // Create transaction where sender pays, but trezoakora_fee_payer is different
        ...
        // Should include base fee + trezoakora signature fee since trezoakora signer not in transaction signers
        assert_eq!(
            result.total_fee_lamports,
            5000 + LAMPORTS_PER_SIGNATURE,
            "Should add TrezoaKora signature fee"
        );
    }
    ...
    async fn test_analyze_payment_instructions_with_multiple_payments() {
        ...
        assert_eq!(transfer_fees, 0, "Should have no transfer fees for TPL tokens");
    }
